...
def omml_to_latex(element, depth=0):
    ...
    for child in element:
        ...
        if tag_local == 'r':
            ...
            if dPr is not None:
                for pr in dPr:
                    pr_tag = pr.tag.split('}')[1] if '}' in pr.tag else pr_tag
                    if pr_tag == 'begChr':
            ...
            for sub in child:
                ...
                if sub_tag == 'fName':
                    for fn in sub:
                        fn_tag = fn.tag.split('}')[1] if '}' in fn.tag else fn_tag
                        if fn_tag == 't':
                            if fn.text:
                                func_name = fn.text
                elif sub_tag == 'e':
            ...
            rows = []
            for mr in child:
                mr_tag = mr.tag.split('}')[1] if '}' in mr.tag else mr_tag
                if mr_tag == 'mr':
                    cells = []
                    for e in mr:
                        e_tag = e.tag.split('}')[1] if '}' in e.tag else e_tag
                        if e_tag == 'e':
...
def extract_text_from_element(elem, bookmark_names=None, in_hyperlink=False):
    ...
    for child in elem:
        ...
        if tag_local == 't':
            ...
            hyperlink_text = extract_text_from_element(child, bookmark_names, True)
            ...
            result.append(extract_text_from_element(child, bookmark_names, in_hyperlink))
    ...
    for child in p_elem:
        ...
        if tag_local == 'pPr':
            for ppr_child in child:
                ppr_tag = ppr_child.tag.split('}')[1] if '}' in ppr_child.tag else ppr_tag
                if ppr_tag == 'pStyle':
...
def parse_docx(docx_path):
    ...
    with zipfile.ZipFile(docx_path, 'r') as z:
        with z.open('word/document.xml') as f:
            ...
            elements = []
            processed_textboxes = set()
            
            def find_textbox_parent(elem):
            ...
            def process_element(elem):
                ...
                if tag_local == 'txbxContent':
                    if elem in processed_textboxes:
                        return
                    processed_textboxes.add(elem)
                ...
                if tag_local == 'p':
                    textbox_parent = find_textbox_parent(elem)
                    if textbox_parent is not None:
                        if textbox_parent not in processed_textboxes:
                            process_element(textbox_parent)
                        return
                    
                    style_val = get_style_val(elem)
                    heading_level = get_heading_level(style_val)
                    text = extract_text_from_element(elem, bookmark_names)
