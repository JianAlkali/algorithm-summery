...
def extract_text_from_element(elem, bookmark_names=None, bookmark_to_title=None):
    if bookmark_names is None:
        bookmark_names = {}
    if bookmark_to_title is None:
        bookmark_to_title = {}
    ...
    for child in elem:
        ...
        if tag_local == 't':
            ...
            hyperlink_text = extract_text_from_element(child, bookmark_names, bookmark_to_title)
            anchor = child.get(qn('w:anchor'))
            if anchor:
                anchor_name = bookmark_names.get(anchor, anchor)
                if anchor_name.startswith('_Toc'):
                    title = bookmark_to_title.get(anchor, anchor_name)
                    result.append(f'[[{title}]]')
                elif anchor_name.startswith('_'):
                    clean_name = anchor_name[1:]
                    result.append(f'[[{clean_name}]]')
                else:
                    result.append(f'[[{anchor_name}]]')
            else:
            ...
            result.append(extract_text_from_element(child, bookmark_names, bookmark_to_title))
...
def extract_textbox_from_drawing(drawing_elem, bookmark_names, bookmark_to_title):
    textbox_paras = []
    ...
    for elem in drawing_elem.iter():
        ...
        if tag_local == 'txbxContent':
            for p in elem:
                ...
                if p_tag == 'p':
                    para_text = extract_text_from_element(p, bookmark_names, bookmark_to_title)
                    if para_text.strip():
    ...
    with zipfile.ZipFile(docx_path, 'r') as z:
        with z.open('word/document.xml') as f:
            ...
            for child in body:
                ...
                if tag_local == 'p':
                    ...
                    for elem in child:
                        ...
                        if elem_tag == 'bookmarkStart':
                            bid = elem.get(qn('w:id'))
                            if bid:
                                current_bookmark = bid
                    
                    style_val = get_style_val(child)
                    heading_level = get_heading_level(style_val)
                    
                    if heading_level is not None and heading_level > 0 and current_bookmark:
                        text = extract_text_from_element(child, bookmark_names, bookmark_to_title)
                        clean_text = clean_title_text(text)
                        bookmark_to_title[current_bookmark] = clean_text
            ...
            def process_paragraph(p_elem):
                ...
                heading_level = get_heading_level(style_val)
                text = extract_text_from_element(p_elem, bookmark_names, bookmark_to_title)
            ...
            def process_drawing(drawing_elem):
                ...
                textbox_content = extract_textbox_from_drawing(drawing_elem, bookmark_names, bookmark_to_title)
                if textbox_content:
                    elements.append({
                        'type': 'textbox',
